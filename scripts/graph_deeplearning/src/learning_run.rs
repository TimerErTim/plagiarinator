...
impl<I, M, O> RunningTraining<I, M, O> {
    pub fn new(model: M, optimizer: O, meta: I, checkpoint_dir: PathBuf) -> Self {
        Self {
            ...
            meta,
            step: 0,
            run_timestamp: chrono::Utc::now().format("%Y%m%d_%H%M%S").to_string(),
            checkpoint_dir,
        }
    }
    ...
    pub fn load_checkpoint<B1: Backend, B2: Backend>(
        mut model: M,
        mut optimizer: O,
    ...
    where
        M: ModuleSnapshot<B1>,
        O: ModuleSnapshot<B2>,
        StoredMetaInformation<I>: DeserializeOwned,
    {
        ...
        let meta_information: StoredMetaInformation<I> = serde_json::from_reader(
            std::fs::File::open(files.meta_information)
                .map_err(|e| CheckpointError::MetaInformationLoadError(e.to_string()))?,
        )
        .map_err(|e| CheckpointError::MetaInformationLoadError(e.to_string()))?;

        let _ = model
            .load_from(&mut model_store)
            .map_err(|e| CheckpointError::ModelLoadError(e.to_string()))?;
        let _ = optimizer
            .load_from(&mut optimizer_store)
            .map_err(|e| CheckpointError::OptimizerLoadError(e.to_string()))?;
    ...
    pub fn load_latest_checkpoint<B1: Backend, B2: Backend>(
        model: M,
        optimizer: O,
        checkpoint_dir: impl AsRef<Path>,
    ) -> Result<Self, CheckpointError>
        ...
        StoredMetaInformation<I>: DeserializeOwned,
    {
        ...
        let latest_checkpoint = match max_step {
            Some(step) => step,
            None => {
                return Err(CheckpointError::LatestCheckpointNotFound(
        ...
        I: Serialize + Clone,
    {
        std::fs::create_dir_all(&self.checkpoint_dir)
            .map_err(|e| CheckpointError::ModelSaveError(e.to_string()))?;
        ...
        let mut optimizer_store =
            burn::store::BurnpackStore::from_file(files.optimizer).overwrite(true);
        let stored_meta_information = StoredMetaInformation::<I> {
            run_timestamp: self.run_timestamp.clone(),
            meta: self.meta.clone(),
        };
        let serialized_meta_information = serde_json::to_string(&stored_meta_information)
            .map_err(|e| CheckpointError::MetaInformationSaveError(e.to_string()))?;
        let serialized_plain_meta = serde_json::to_string(&self.meta)
            .map_err(|e| CheckpointError::MetaInformationSaveError(e.to_string()))?;

        model_store = model_store
            .metadata("run_timestamp", self.run_timestamp.clone())
            .metadata("step", self.step.to_string())
            .metadata("meta", serialized_plain_meta.clone());
        optimizer_store = optimizer_store
            .metadata("run_timestamp", self.run_timestamp.clone())
            .metadata("step", self.step.to_string())
            .metadata("meta", serialized_plain_meta);

        self.model
            .save_into(&mut model_store)
            .map_err(|e| CheckpointError::ModelSaveError(e.to_string()))?;
        self.optimizer
            .save_into(&mut optimizer_store)
            .map_err(|e| CheckpointError::OptimizerSaveError(e.to_string()))?;
        std::fs::write(files.meta_information, serialized_meta_information)
            .map_err(|e| CheckpointError::MetaInformationSaveError(e.to_string()))?;
    ...
    fn checkpoint_files(checkpoint_dir: impl AsRef<Path>, step: usize) -> CheckpointFiles {
        CheckpointFiles {
            ...
            optimizer: checkpoint_dir
                .as_ref()
                .join(format!("optimizer_{step}.bpk")),
            meta_information: checkpoint_dir
                .as_ref()
                .join(format!("meta_information_{step}.json")),
        }
    }
}
