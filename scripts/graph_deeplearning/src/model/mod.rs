...
use burn::{module::Module, prelude::Backend, tensor::{Int, activation::softmax, backend::AutodiffBackend}};
pub use graph::*;
pub use ast::*;
use thiserror::Error;
...
pub fn analyze_plagiarism<B: AutodiffBackend>(file_1: ParsedFile, file_2: ParsedFile, model: PlagiarismDecider<B>) -> Result<PlagiarismInsights, PlagiarismAnalysisError>
where B::FloatElem: Into<f64> {
    ...
    let graph_1 = Graph::from_flattened_ast(&file_1.ast, model_device).ok_or(PlagiarismAnalysisError::EmptyAST)?;
    let graph_2 = Graph::from_flattened_ast(&file_2.ast, model_device).ok_or(PlagiarismAnalysisError::EmptyAST)?;
    ...
    let node_1_importance = softmax(embedded_nodes_1.grad_remove(&mut grads).unwrap().abs().sum_dim(1), 0);
    let node_2_importance = softmax(embedded_nodes_2.grad_remove(&mut grads).unwrap().abs().sum_dim(1), 0);
    ...
    let insights = PlagiarismInsights {
        file_1: AnalyzedFile { 
            ...
            ast_nodes: file_1.ast.nodes.into_iter().zip(node_1_importance.iter_dim(0).map(|weight_t| weight_t.into_scalar().into())).map(|(node, weight)| (weight, node)).collect(),
        },
        file_2: AnalyzedFile { 
            file_path: file_2.file_path,
            file_content: file_2.file_content,
            ast_nodes: file_2.ast.nodes.into_iter().zip(node_2_importance.iter_dim(0).map(|weight_t| weight_t.into_scalar().into())).map(|(node, weight)| (weight, node)).collect(),
        },
