...
int main() {
    ...
    std::string buf;
    while (std::getline(stdinCopy, buf)) {
        std::size_t commaPos = buf.find(',');
        lines.emplace_back(buf.substr(0, commaPos),
                          std::stoi(buf.substr(commaPos + 1)));
    }
