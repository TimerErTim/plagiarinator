...
void do_postfix(const std::string& text) {
    std::vector<Expr> stk;
    ...
    while (in >> piece) {
        if (is_op(piece)) {
            ...
            int lv = compute(lhs);
            char op = piece[0];
            if (op == '+') stk.push_back(Add{lv, rv});
            else if (op == '-') stk.push_back(Sub{lv, rv});
            else if (op == '*') stk.push_back(Mul{lv, rv});
            else stk.push_back(Div{lv, rv});
        } else {
            stk.push_back(Number{std::stoi(piece)});
        }
    }
    if (stk.empty()) return;
    try {
        std::cout << compute(stk.back()) << '\n';
    } catch (...) {
        std::cout << "ERROR\n";
    }
}
