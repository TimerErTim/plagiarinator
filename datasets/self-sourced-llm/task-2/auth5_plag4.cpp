...
public:
    ...
    ~LinkedList() {
        Node* p = h;
        while (p) { Node* d = p; p = p->next; delete d; }
    }
...
int main() {
    ...
    { std::istringstream in(s); int x; while (in >> x) L1.push_back(x); }
    std::getline(std::cin, s);
    { std::istringstream in(s); int x; while (in >> x) L2.push_back(x); }

    LinkedList<int> M = merge_sorted(L1, L2);
    bool sep = false;
