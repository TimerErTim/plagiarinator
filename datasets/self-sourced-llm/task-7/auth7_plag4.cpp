...
std::unique_ptr<Vehicle> create_vehicle(const std::string& kind, std::istream& in) {
    if (kind=="CAR"){double x;in>>x;return std::make_unique<Car>(x);}
    if (kind=="TRUCK"){double x;in>>x;return std::make_unique<Truck>(x);}
...
int main() {
    ...
    std::string k;
    while(std::cin>>k) {
        auto numP=create_vehicle(k,std::cin);
        if(!numP){std::string rest;std::getline(std::cin,rest);std::cout<<"ERROR\n";continue;}
        double d;std::cin>>d;
        V.push_back(std::move(numP)); D.push_back(d);
    }
    double s=0;
