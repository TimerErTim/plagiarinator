...
public:
    ...
    void push(T value) {
        ...
        not_full_.wait(lock, [this] { return body_.size() < capacity_; });
        body_.push(std::move(value));
        not_empty_.notify_one();
    }

    T pop() {
        std::unique_lock<std::mutex> lock(guard_);
        not_empty_.wait(lock, [this] { return !body_.empty(); });
        T val = std::move(body_.front());
        body_.pop();
...
int main() {
    ...
    std::string ln;
    std::cin >> ln >> num_prod >> ln >> num_cons >> ln >> num_items;
