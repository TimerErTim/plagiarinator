#include <iostream>
#include <utility>
// cleaned up version

template <typename T>
class Vector {
public:
    Vector() : ptr_(nullptr), sz_(0), limit_(0) {}
    ~Vector() { delete[] ptr_; }

    Vector(const Vector& o) : sz_(o.sz_), limit_(o.limit_) {
        ptr_ = limit_ ? new T[limit_] : nullptr;
        for (std::size_t i = 0; i < sz_; ++i) ptr_[i] = o.ptr_[i];
    }

    Vector& operator=(const Vector& o) {
        if (this != &o) {
            Vector tmp(o);
            std::swap(ptr_, tmp.ptr_);
            std::swap(sz_, tmp.sz_);
            std::swap(limit_, tmp.limit_);
        }
        return *this;
    }

    void push_back(const T& v) {
        maybe_grow();
        ptr_[sz_++] = v;
    }

    void push_back(T&& v) {
        maybe_grow();
        ptr_[sz_++] = std::move(v);
    }

    T& operator[](std::size_t i) { return ptr_[i]; }
    const T& operator[](std::size_t i) const { return ptr_[i]; }

    std::size_t size() const { return sz_; }
    T* data() { return ptr_; }
    const T* data() const { return ptr_; }

private:
    T* ptr_;
    std::size_t sz_, limit_;

    void maybe_grow() {
        if (sz_ < limit_) return;
        std::size_t ncap = limit_ ? limit_ * 2 : 4;
        T* nptr = new T[ncap];
        for (std::size_t i = 0; i < sz_; ++i) nptr[i] = ptr_[i];
        delete[] ptr_;
        ptr_ = nptr;
        limit_ = ncap;
    }
};

int main() {
    std::size_t cnt;
    if (!(std::cin >> cnt)) return 0;

    Vector<int> src;
    for (std::size_t i = 0; i < cnt; ++i) {
        int n;
        std::cin >> n;
        src.push_back(n);
    }

    Vector<int> uniq;
    for (std::size_t i = 0; i < src.size(); ++i) {
        bool hit = false;
        for (std::size_t j = 0; j < uniq.size(); ++j)
            if (uniq[j] == src[i]) { hit = true; break; }
        if (!hit) uniq.push_back(src[i]);
    }

    long long s = 0, p = 1;
    const long long M = 1000000007LL;
    for (std::size_t i = 0; i < uniq.size(); ++i) {
        s += uniq[i];
        p = (p * uniq[i]) % M;
    }

    std::cout << "SUM " << s << '\n';
    std::cout << "PROD " << p << '\n';
    return 0;
}
