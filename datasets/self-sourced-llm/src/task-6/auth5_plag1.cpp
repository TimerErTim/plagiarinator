...
int main() {
    ...
    while (std::cin >> t && t != "DONE") {
        if (t == "CIRCLE") { double r; std::cin >> r; v.emplace_back(std::make_unique<Circle>(r)); }
        else if (t == "RECT") { double w,h; std::cin >> w >> h; v.emplace_back(std::make_unique<Rectangle>(w,h)); }
        else if (t == "TRI") { double s; std::cin >> s; v.emplace_back(std::make_unique<Triangle>(s)); }
    }
    double A=0,P=0;
