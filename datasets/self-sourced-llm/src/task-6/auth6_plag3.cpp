...
class Circle : public Shape {
    double rVal;
public:
    explicit Circle(double r) : rVal(r) {}
    double area() const override {
        return 3.141592653589793 * rVal * rVal;
    }
    double perimeter() const override {
        return 2.0 * 3.141592653589793 * rVal;
    }
...
class Triangle : public Shape {
    double sVal;
public:
    explicit Triangle(double s) : sVal(s) {}
    double area() const override {
        return std::sqrt(3.0) / 4.0 * sVal * sVal;
    }
    double perimeter() const override { return 3.0 * sVal; }
