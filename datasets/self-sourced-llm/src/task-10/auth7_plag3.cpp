...
int main(){
    IntBST hold; std::string s;
    while(std::cin>>s){if(s=="DONE")break;hold.insert(std::stoi(s));}
    while(std::cin>>s){
        if(s=="DONE")break;
        if(s=="CONTAINS"){int x;std::cin>>x;std::cout<<(hold.contains(x)?"YES":"NO")<<"\n";}
        else if(s=="MIN"){std::cout<<(hold.empty()?"EMPTY":std::to_string(hold.min()))<<"\n";}
        else if(s=="PRINT"){std::vector<int> v;hold.inorder(v);for(size_t i=0;i<v.size();++i){if(i)std::cout<<' ';std::cout<<v[i];}std::cout<<"\n";}
    }
    return 0;
}
