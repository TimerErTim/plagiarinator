...
{
    Parser machine;
    machine.state.reset(new HeaderState());
    std::string input;
    while (std::getline(std::cin, input))
        machine.state = machine.state->handle(machine, input);

    std::cout << "INFO: " << machine.info << '\n';
    std::cout << "WARN: " << machine.warn << '\n';
    std::cout << "ERROR: " << machine.err << '\n';
