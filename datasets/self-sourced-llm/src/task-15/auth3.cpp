...
int main() {
    ...
    for (std::string type; std::cin >> type && type != "END";) {
        double v;
        std::cin >> v;
        if (type == "DIST") rows.emplace_back(metrics::Distance(v));
        else if (type == "MASS") rows.emplace_back(metrics::Mass(v));
        else if (type == "TEMP") rows.emplace_back(metrics::Temperature(v));
    }
