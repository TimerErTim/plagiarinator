...
int main()
{
    ...
    while(std::cin>>s)
{if(s=="DONE")break;t.insert(std::stoi(s));}
    while(std::cin>>s)
{
        if(s=="DONE")break;
        if(s=="CONTAINS")
{int x;std::cin>>x;std::cout<<(t.contains(x)?"YES":"NO")<<"\n";}
        else if(s=="MIN")
{std::cout<<(t.empty()?"EMPTY":std::to_string(t.min()))<<"\n";}
        else if(s=="PRINT")
{std::vector<int> v;t.inorder(v);for(size_t i=0;i<v.size();++i){if(i)std::cout<<' ';std::cout<<v[i];}std::cout<<"\n";}
    }
    return 0;
}
